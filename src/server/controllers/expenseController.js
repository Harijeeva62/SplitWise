...
const CATEGORIES = ['General', 'Food', 'Transport', 'Stay', 'Shopping', 'Entertainment', 'Bills', 'Other'];

exports.addExpense = async (req, res) => {
  try {
    const { group_id, title, amount, paid_by, split_between, split_amounts, category, notes, date } = req.body;
    ...
    // Build splits — support unequal amounts
    let splits;
    if (split_amounts && Array.isArray(split_amounts) && split_amounts.length > 0) {
      // Custom split: [{ user_id, amount }]
      splits = split_amounts.map(s => ({
...
exports.getExpensesByGroup = async (req, res) => {
  try {
    ...
    // Calculate group total and category breakdown
    let groupTotal = 0;
    ...
    res.json({
      expenses: enriched,
      group_total: parseFloat(groupTotal.toFixed(2)),
...
exports.getDashboard = async (req, res) => {
  try {
    ...
    if (!memberships || memberships.length === 0) {
      return res.json({ total_owed: 0, total_owe: 0, recent_expenses: [], category_totals: {} });
    }
    ...
    const { data: expenses } = await supabase
      .from('expenses')
      .select('id, amount, paid_by, group_id, title, created_at, category')
